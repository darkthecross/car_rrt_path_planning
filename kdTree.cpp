...
template <class T, int N> kdTreeNode<T> *kdTree<T, N>::insert(T newVal) {
  if (this->root == NULL) {
...
template <class T, int N> kdTreeNode<T> *kdTree<T, N>::findNearest(T newVal) {
  if (root == NULL) {
    return NULL;
  } else {
    T lower, upper;
    ...
    return findNearestHelper(newVal, root, lower, upper,
                             std::numeric_limits<double>::max(), 0);
  }
}

template <class T, int N>
kdTreeNode<T> *kdTree<T, N>::findNearestHelper(T newVal, kdTreeNode<T> *curNode,
                                               T lowerB, T upperB,
                                               double cur_min, int dps) {
  if (curNode == NULL) {
    return NULL;
  }
  ...
  if (curNode->left == NULL && curNode->right == NULL) {
    ...
    if (curNode->left == NULL) {
      kdTreeNode<T> *N1 = findNearestHelper(newVal, curNode->right, lowerB1,
                                            upperB, cur_min1, dps + 1);
      return selectNN(newVal, N1, curNode);
    } else if (curNode->right == NULL) {
      kdTreeNode<T> *N1 = findNearestHelper(newVal, curNode->left, lowerB,
                                            upperB1, cur_min1, dps + 1);
      return selectNN(newVal, N1, curNode);
    } else {
      kdTreeNode<T> *N1 = findNearestHelper(newVal, curNode->right, lowerB1,
                                            upperB, cur_min1, dps + 1);
      kdTreeNode<T> *N2 = findNearestHelper(newVal, curNode->left, lowerB,
                                            upperB1, cur_min1, dps + 1);
...
template <class T, int N>
kdTreeNode<T> *kdTree<T, N>::insertHelper(T newVal, kdTreeNode<T> *subTreeRoot,
                                          int dps) {
  int cd = dps % N;
...
double kdTree<T, N>::lowerBoundInRange(T newVal, T lower, T upper) {
  T possibleNN;
  for (int i = 0; i < N; i++) {
    possibleNN.push_back(
        (newVal[i] < lower[i])
            ? (lower[i])
            : (newVal[i] > upper[i] ? (upper[i]) : (newVal[i])));
  }
  return NdDist(newVal, possibleNN);
}

template <class T, int N> double kdTree<T, N>::NdDist(T N1, T N2) {
  double res = 0;
  for (int i = 0; i < N; i++) {
...
template <class T, int N>
kdTreeNode<T> *kdTree<T, N>::selectNN(T newVal, kdTreeNode<T> *N1,
                                      kdTreeNode<T> *N2) {
  if (N1 == NULL) {
    return N2;
  } else if (N2 == NULL) {
...
template class crpp::kdTree<std::vector<double>, 2>;
