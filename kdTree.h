...
template <class T> struct kdTreeNode {
  T val;
  kdTreeNode *left;
  kdTreeNode *right;
  kdTreeNode *parent;
  ...
  kdTreeNode(T ele) {
    ...
    this->right = NULL;
    this->parent = NULL;
    this->children = std::vector<kdTreeNode *>();
  };
  kdTreeNode() {
    this->val = T();
    this->left = NULL;
...
template <class T, int N> class kdTree {
private:
  kdTreeNode<T> *findNearestHelper(T newVal, kdTreeNode<T> *curNode, T lowerB,
                                   T upperB, double cur_min, int dps);
